...
```
gzip -d openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img.gz
```
2. Run 
```
VBoxManage convertfromraw --format VDI openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.vdi
```
...
```
dd if=openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img of=openwrt.img bs=128000 conv=sync
```

followed by:

```
VBoxManage convertfromraw --format VDI openwrt.img openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.vdi
```

Finally, enlarge the image:

```
VBoxManage modifymedium openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.vdi --resize 128
```
...
2. Once written, the SYSLINUX partition on the USB Drive that you've just written should be mountable; put the Gzipped image (e.g. openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img.gz) onto it. Unmount the USB drive, disconnect it from your computer and connect it to the APU2.
...
5. * Copy and unzip the image
    ```
    ~ # cp /media/SYSLINUX/openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img.gz .
    ~ # gunzip openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img.gz
    ```
    
    * Apply the image. Run `lsblk` to choose the correct device to which to write:
    ```
    ~ # dd if=openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img of=/dev/sda bs=4M; sync
    5+1 records in
    5+1 records out
    ```
...
5. Once in the rescue shell:
    ...
    * Download and unzip the image
    ```
    ~ # wget https://raw.githubusercontent.com/byteandnibble/apu2_openwrt/master/bin/21.02.0/openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img.gz
    ~ # gunzip openwrt-21.02.3-byteandnibble-x86-64-generic-squashfs-combined.img.gz
    ```
