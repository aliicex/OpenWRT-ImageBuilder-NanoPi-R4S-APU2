# OpenWRT ImageBuilder for the FriendlyARM NanoPi R4S, the PC Engines apu2 platform, and the Netgear WAC124 & WAX202

Creates the environment required to build the images for my network devices: FriendlyARM NanoPi R4S, the PC Engines apu2 platform, and the Netgear WAC124 & WAX202 (as a dumb APs) The following packages are included for the routers:
...
4. You are now in the rescue setup. You should see `Rescue mode` at the time. Choose your language, country, etc. Skip loading any firmware, and choose `enp1s0` as your primary ethernet if prompted. When it asks if you want to choose a root filesystem device, _scroll down_ and choose "Do not use a root file system". Finally, you should be able to execute the rescue shell.
